...
# A refreshed sudo ticket is not enough: Homebrew casks and the dotfile
# symlinks into /usr/local/bin still prompt partway through the run, so
# grant this user passwordless sudo for the duration instead
